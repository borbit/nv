  ...
  function onload(err, assets) {
    ...
    try {
      if (op.lang || config.LOCALES) {
        i18n.setLocale(op.lang || config.LOCALE_DEFAULT);
        moment.lang(op.lang || config.LOCALE_DEFAULT);
      }
    } catch (err) {
      return cb(new Error('Cannot load locale "' + op.lang + '"'))
    }
