...
module.exports = function(options, cfg, cb) {
  ...
  function renderPage(req, res) {
    ...
    loadPageData(page, function(err, data) {
      try {
        res.render(tpl, data);
      } catch(err) {
        res.end('404');
  ...
  function loadProdAssets(page, cb) {
    utils.loadJSON(cfg.VERSION_FILE, function(err, version) {
      var assetsFilePath = utils.getAssetsFilePath(cfg.DISTS_ROOT, version);
      var imagesFilePath = utils.getImagesFilePath(cfg.DISTS_ROOT, version);;
