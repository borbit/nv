...
, STATIC_ROOT     : path.join(cwd, 'static')
, TEMPLATE_ENGINE : 'ejs'
...
, LOCALES         : false
, LOCALE_DEFAULT  : 'en'
, LOCALES_ROOT    : path.join(cwd, 'i18n')
, LOCALES_LIST    : ['en']
};
 ...
 *
 */
nv.loadDevAssets = function(page, cb) {
  async.series({
    cmn: function(cb) {
 ...
 *
 */
nv.loadFlattenDevAssets = function(page, cb) {
  async.series({
    cmn: function(cb) {
      nv.loadJSON(nv.getCmnAssetsPath(), cb);
    },
...
nv.loadProdAssets = function(page, cb) {
  nv.loadJSON(config.VERSION_FILE, function(err, version) {
    ...
    var mocksFilePath = nv.getPageMocksPath(page);
