...
utils.getAssetsFilePath = function(rootPath, version) {
  return path.join(rootPath, version, 'assets.json')
};

utils.getImagesFilePath = function(rootPath, version) {
  return path.join(rootPath, version, 'images.json')
};
 ...
 *
 */
utils.loadProdAssets = function(assetsFilePath, imagesFilePath, mocksPagePath, cb) {
  utils.loadJSON(assetsFilePath, function(err, assets) {
    utils.loadJSON(imagesFilePath, function(err, images) {
      utils.loadMocks(mocksPagePath, function(err, mocks) {
        cb(null, {assets: assets, images: images, mocks: mocks});
      });
    });
  });
};
